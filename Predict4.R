...
load("Data/PML2.RData")
bigram[,pml:=PML2]

load("Data/PML3.RData")
trigram[,pml:=PML3]

load("Data/PML4.RData")
foregram[,pml:=PML4]

sum_unigram_count <- 24576915
most_freq <-unigram[order(-count),][1:5] # unigram 5 most freq
most_freq[,pml:=count/sum_unigram_count]
most_freq[,layer:=1]
most_freq[,back_id:=0]
most_freq[,back_word:=""]
#most_freq[,pwb:=0]
...
predict <- function(query) {
    ...
    if (size==2) { # predict by trigram #######################################
        ...
        if (nrow(res3)<5) {
            ...
            res <- rbind(res3[order(-res3$pml),],res2[order(-res2$pml),])
            
        } else {
            res <- res3[order(-res3$pml),]
        }
